...
版本：`1.0.0-product-detail`

正式品牌：`中科固源 / SOURCEGUARD`

主题：Light only

当前状态：完整 Red Hat 原始色板与本地语义映射已实现；Button、Link、Call to action、Marketing CTA band、Skip Link、Desktop Navigation、Mobile Menu、Global Header 与 Footer 已完成 canonical component 合约；Product detail 与 Page ending patterns 已完成，其他组件与 patterns 仍需逐项审核。
...
| Site components | `src/components/site/`                     | Mixed       | Navigation, Header, Footer, and CTA band ready      |
| Patterns        | `docs/design-system/patterns/`             | Mixed       | Product detail and Page ending are Implemented      |
